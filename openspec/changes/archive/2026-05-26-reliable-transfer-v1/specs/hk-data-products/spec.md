...
transfers to traverse the `reliable-transfer-v1` COMM sidecar on the default
S-band node-`5` path.

#### Scenario: Official HK `.fdp` generation remains unchanged

- **WHEN** `reliable-transfer-v1` is enabled for a selected official HK `.fdp`
  request
...
- **AND** `DpCatalog` SHALL still own catalog build and selected transmit
  initiation
- **AND** the change SHALL only alter the active file-delivery behavior after
  `CommController` accepts the request

#### Scenario: Completion truth remains whole-file at the catalog boundary

- **WHEN** a selected official HK `.fdp` request completes through the reliable
  path
...
#### Scenario: V1 ceiling remains local and bounded

- **WHEN** a selected official HK `.fdp` request exceeds the
  `reliable-transfer-v1` local transfer ceiling of `64` segments / `10,240`
  bytes
- **THEN** `CommController` SHALL reject the reliable-transfer start before
  in-transfer delivery begins
- **AND** node-`5` reliable-transfer reception SHALL reject a `BEGIN` that
  advertises a larger file or segment count
