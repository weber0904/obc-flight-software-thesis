...
- [x] 1.1 Initialize `change-closeout` and `hosted-probe-workflow` under `.codex/skills/` using the official Codex skill tooling.
- [x] 1.2 Author both `SKILL.md` files so their trigger text and workflow match the repository's recent repeated patterns.
- [x] 1.3 Generate Codex-compatible `agents/openai.yaml` metadata for both skills.
