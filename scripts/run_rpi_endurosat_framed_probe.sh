...
RUNTIME_ROOT="${RUNTIME_ROOT:-${RPI_REMOTE_DIR}/runtime/integ-rpi-endurosat-framed}"
HOST_PEER_LOG="${HOST_PEER_LOG:-/tmp/obc-host-framed-peer.log}"
...
if [[ -z "${HOST_SERIAL_DEVICE}" || -z "${RPI_COMM_DEVICE}" ]]; then
  echo "Usage: bash scripts/run_rpi_endurosat_framed_probe.sh /path/to/host-serial-device /dev/serial0" >&2
...
if ! grep -q 'transparent-framed-echo frame=' "${HOST_PEER_LOG}"; then
  echo "Host framed peer did not observe any framed payloads" >&2
  cat "${HOST_PEER_LOG}" >&2 || true
  exit 1
fi

grep 'transparent-framed-echo frame=' "${HOST_PEER_LOG}"
