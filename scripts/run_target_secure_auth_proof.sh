...
TARGET_CAN_PROBE="${ROOT_DIR}/scripts/comm_verification/lib/run_target_can_matrix_probe.py"
...
probe_root="${PROBE_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/target-secure-auth-proof-v1.XXXXXX")}"
proof_root="${probe_root}/target-secure-auth-proof-v1"
...
run_target_secure_auth_proof() {
  ...
  env \
    ...
    "${PYTHON_BIN}" "${TARGET_CAN_PROBE}" --mode secure-auth-proof --profile uhf-primary --probe-root "${proof_root}"
}
...
: >"${summary_log}"
log_line "target-secure-auth-proof-v1: START"
log_line "probe-root=${probe_root}"
probe_output="$(mktemp "${TMPDIR:-/tmp}/target-secure-auth-proof-output.XXXXXX")"
cleanup_probe_output() {
  rm -f "${probe_output}"
}
trap cleanup_probe_output EXIT
if ! run_target_secure_auth_proof >"${probe_output}" 2>&1; then
...
log_line "target-secure-auth-proof-v1: PASS"
log_line "proof-root=${proof_root}"
