...
RUNTIME_ROOT="${RUNTIME_ROOT:-${RPI_REMOTE_DIR}/runtime/integ-rpi-uart-probe}"
HOST_PEER_LOG="${HOST_PEER_LOG:-/tmp/obc-host-serial-peer.log}"
...
if [[ -z "${HOST_SERIAL_DEVICE}" || -z "${RPI_COMM_DEVICE}" ]]; then
  echo "Usage: bash scripts/run_rpi_uart_probe.sh /path/to/host-serial-device /dev/serial0" >&2
...
"${BIN_DIR}/radio_mock_server" --serial-device "${HOST_SERIAL_DEVICE}" --baudrate "${COMM_BAUDRATE}" >"${HOST_PEER_LOG}" 2>&1 &
HOST_PEER_PID=$!
