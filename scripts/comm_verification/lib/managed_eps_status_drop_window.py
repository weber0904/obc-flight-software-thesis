...
def send_eps_drop_status_command(subsystem_target: str, control_socket_path: str, count: int) -> str:
    script = """
import socket
import sys

socket_path = sys.argv[1]
command = f"drop-status {int(sys.argv[2])}\\n".encode("utf-8")
...
def drive_managed_eps_status_drop_window(
    *,
    obc_target: str,
    obc_service: str,
    ...
    try:
        payload["controlResponse"] = send_eps_drop_status_command(subsystem_target, control_socket_path, drop_count)
        ...
        while time.time() < deadline:
            ...
            if all(fragment in journal for fragment in journal_fragments):
                ...
                payload["triggerSource"] = "eps-control-socket+target-journal"
        ...
        if payload.get("verdict") != "PASS":
            ...
            raise ProbeFailure("timed out waiting for EPS recovery journal markers after status-drop injection")
    finally:
        try:
            payload["cleanupControlResponse"] = send_eps_drop_status_command(subsystem_target, control_socket_path, 0)
