...
PROBE_TMP_DIR="${PROBE_TMP_DIR:-$(mktemp -d "/tmp/obc-sband-ccsds-primary.XXXXXX")}"
...
GDS_LOG_DIR = PROBE_TMP_DIR / "gds-logs"
GDS_WRAPPER_LOG = PROBE_TMP_DIR / "gds-wrapper.log"
...
COMM_NODE_LOG = PROBE_TMP_DIR / "sband-comm-node.log"
...
def main() -> None:
    ...
    try:
        events_handle = EVENTS_LOG.open("w", encoding="utf-8", buffering=1)
        obc_handle = OBC_LOG.open("w", encoding="utf-8", buffering=1)
        try:
            start_process(
                "fprime-gds",
                [
            ...
            start_process(
                "sband-comm-csp-node",
                [str(BIN_DIR / "sband_comm_csp_node"), "--tcp-listen-host", "127.0.0.1", "--tcp-listen-port", str(sband_port), "--node-id", "5"],
                COMM_NODE_LOG,
                env=env,
                stale_match_groups=(
                    build_fragment_group(
                        str(BIN_DIR / "sband_comm_csp_node"),
                        f"--tcp-listen-port {sband_port}",
                        "--node-id 5",
                    ),
                ),
            )
            start_process(
                "ground-ttc-gateway",
                [str(BIN_DIR / "ground_ttc_gateway"), "--rf-tcp-host", "127.0.0.1", "--rf-tcp-port", str(sband_port), "--link-identity", "sband", "--gds-host", "127.0.0.1", "--gds-port", str(gds_port)],
                GATEWAY_LOG,
                env=env,
                stale_match_groups=(
                    build_fragment_group(
                        str(BIN_DIR / "ground_ttc_gateway"),
                        f"--rf-tcp-port {sband_port}",
                        "--link-identity sband",
                        f"--gds-port {gds_port}",
                    ),
                ),
            )
            wait_port(sband_port, READY_TIMEOUT)
            time.sleep(1.0)
            ...
            wait_for_status(
                ...
                obc_lines,
                [
                    "comm primary command=SBAND",
                    "sbandAvailable=yes",
                    "sbandReason=HEALTHY_ACTIVITY",
                    "groundLink mode=comm-csp commNode=5",
                    "groundLink connected=yes",
                ],
                READY_TIMEOUT,
            )

            wait_text(EVENTS_LOG, "Framework Version", READY_TIMEOUT)
            wait_text(EVENTS_LOG, "COMM_LINK_AVAILABILITY_CHANGED", READY_TIMEOUT)
            wait_for_status(
                obc,
                obc_lines,
                [
