...
TARGET_PROBE="${ROOT_DIR}/scripts/comm_verification/lib/run_payload_target_capture_sanity_v1_target_probe.py"
...
probe_root="${PROBE_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/payload-target-capture-sanity-v1.XXXXXX")}"
...
TARGET_BASELINE_FORCE_OBC_COMM_RESTART="${TARGET_BASELINE_FORCE_OBC_COMM_RESTART:-1}"
...
OBC_GROUNDLINK_DIAGNOSTICS="${OBC_GROUNDLINK_DIAGNOSTICS:-0}"
...
SBAND_COMM_NODE_INGRESS_DIAGNOSTICS="${SBAND_COMM_NODE_INGRESS_DIAGNOSTICS:-0}"

run_probe() {
  JSON_OUT="${baseline_dir}/target-before.json" \
    TARGET_BASELINE_FORCE_OBC_COMM_RESTART="${TARGET_BASELINE_FORCE_OBC_COMM_RESTART}" \
...
: >"${summary_log}"
log_line "payload-target-capture-sanity-v1: START"
log_line "probe-root=${probe_root}"
probe_output="$(mktemp "${TMPDIR:-/tmp}/payload-target-capture-sanity-v1-output.XXXXXX")"
...
log_line "payload-target-capture-sanity-v1: PASS"
