...
TARGET_SECURE_AUTH_COMMAND_PATH_PROBE="${ROOT_DIR}/scripts/comm_verification/lib/run_target_secure_auth_command_path_probe.py"
...
probe_root="${PROBE_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/target-secure-auth-command-path.XXXXXX")}"
...
run_target_secure_auth_command_path_probe() {
  ...
  env \
    ...
    "${PYTHON_BIN}" "${TARGET_SECURE_AUTH_COMMAND_PATH_PROBE}" --probe-root "${probe_root}"
}
...
: >"${summary_log}"
log_line "target-secure-auth-command-path: START"
log_line "probe-root=${probe_root}"
probe_output="$(mktemp "${TMPDIR:-/tmp}/target-secure-auth-command-path-probe-output.XXXXXX")"
cleanup_probe_output() {
  rm -f "${probe_output}"
}
trap cleanup_probe_output EXIT
if ! run_target_secure_auth_command_path_probe >"${probe_output}" 2>&1; then
...
log_line "target-secure-auth-command-path: PASS"
