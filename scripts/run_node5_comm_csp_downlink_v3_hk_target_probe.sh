...
TARGET_PROBE="${ROOT_DIR}/scripts/comm_verification/lib/run_node5_comm_csp_downlink_v3_hk_target_probe.py"
...
probe_root="${PROBE_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/node5-comm-csp-downlink-v3-hk-target.XXXXXX")}"
summary_log="${probe_root}/summary.log"
baseline_dir="${probe_root}/baseline"

mkdir -p "${probe_root}" "${baseline_dir}"
...
COMM_GROUNDLINK_DOWNLINK_V3_SEND_PROBE_WAIT_MS="${COMM_GROUNDLINK_DOWNLINK_V3_SEND_PROBE_WAIT_MS:-}"
COMM_GROUNDLINK_DOWNLINK_V3_SEND_PROBE_RETRY_SLEEP_MS="${COMM_GROUNDLINK_DOWNLINK_V3_SEND_PROBE_RETRY_SLEEP_MS:-}"
...
HK_TREND_TARGET_FILE_BYTES="${HK_TREND_TARGET_FILE_BYTES:-10240}"
HK_TREND_MIN_FDP_BYTES="${HK_TREND_MIN_FDP_BYTES:-8192}"
HK_TREND_PENDING_WAIT_TIMEOUT_SEC="${HK_TREND_PENDING_WAIT_TIMEOUT_SEC:-240}"
HK_TREND_REUSE_EXISTING_SOURCE="${HK_TREND_REUSE_EXISTING_SOURCE:-0}"
...
run_probe() {
  ...
  env \
    ...
    HK_TREND_TARGET_FILE_BYTES="${HK_TREND_TARGET_FILE_BYTES}" \
    HK_TREND_MIN_FDP_BYTES="${HK_TREND_MIN_FDP_BYTES}" \
    HK_TREND_PENDING_WAIT_TIMEOUT_SEC="${HK_TREND_PENDING_WAIT_TIMEOUT_SEC}" \
    HK_TREND_REUSE_EXISTING_SOURCE="${HK_TREND_REUSE_EXISTING_SOURCE}" \
...
: >"${summary_log}"
log_line "node5-comm-csp-downlink-v3-hk-target: START"
log_line "probe-root=${probe_root}"
probe_output="$(mktemp "${TMPDIR:-/tmp}/node5-comm-csp-downlink-v3-hk-target-output.XXXXXX")"
cleanup_probe_output() {
  rm -f "${probe_output}"
}
trap cleanup_probe_output EXIT
if ! run_probe >"${probe_output}" 2>&1; then
...
log_line "node5-comm-csp-downlink-v3-hk-target: PASS"
