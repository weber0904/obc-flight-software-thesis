...
PROBE_ROOT="${PROBE_ROOT:-$(mktemp -d "/private/tmp/chapter5-route3-target-eps.XXXXXX")}"
SUMMARY_LOG="${SUMMARY_LOG:-${PROBE_ROOT}/summary.log}"
baseline_dir="${PROBE_ROOT}/baseline"

mkdir -p "${baseline_dir}"
chapter5_init_summary "${SUMMARY_LOG}"
chapter5_log_line "${SUMMARY_LOG}" "chapter5-route3-target-eps-r3-safe: START"
...
env TARGET_BASELINE_MANAGED_EXTERNALLY=1 TARGET_BASELINE_REQUIRE_UHF_SERVICE="${TARGET_BASELINE_REQUIRE_UHF_SERVICE}" \
  ...
  --probe-root "${PROBE_ROOT}/eps-r3-safe" \
  --stage eps-r3-safe | tee -a "${SUMMARY_LOG}"
...
chapter5_log_line "${SUMMARY_LOG}" "chapter5-route3-target-eps-r3-safe: PASS"
