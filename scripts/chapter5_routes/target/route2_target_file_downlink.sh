...
probe_root="${PROBE_ROOT:-$(mktemp -d "/private/tmp/chapter5-route2-target-file.XXXXXX")}"
summary_log="${SUMMARY_LOG:-${probe_root}/summary.log}"
baseline_dir="${probe_root}/baseline"
proof_root="${probe_root}/file-downlink-proof"

mkdir -p "${baseline_dir}"
: >"${summary_log}"

JSON_OUT="${baseline_dir}/target-before.json" TARGET_BASELINE_REQUIRE_UHF_SERVICE=1 bash "${ENSURE_TARGET_BASELINE}" >>"${summary_log}" 2>&1
...
env \
  ...
  "${PYTHON_BIN}" "${TARGET_CAN_PROBE}" --mode file-downlink --profile uhf-primary --probe-root "${proof_root}" | tee -a "${summary_log}"
