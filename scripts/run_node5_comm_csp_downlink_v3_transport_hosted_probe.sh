...
COMM_DOWNLINK_V3_SENDER_BIN="${BIN_DIR}/comm_downlink_v3_sender"
COMM_DOWNLINK_V3_STATUS_PROBE_BIN="${BIN_DIR}/comm_downlink_v3_status_probe"
CSP_ZMQPROXY_BIN="${BIN_DIR}/csp_zmqproxy"
SBAND_COMM_NODE_BIN="${BIN_DIR}/sband_comm_csp_node"
for required_bin in \
  "${COMM_DOWNLINK_V3_SENDER_BIN}" \
  "${COMM_DOWNLINK_V3_STATUS_PROBE_BIN}" \
  "${CSP_ZMQPROXY_BIN}" \
...
PROBE_TMP_DIR="${PROBE_TMP_DIR:-$(mktemp -d "/tmp/node5-comm-csp-downlink-v3-transport-hosted.XXXXXX")}"
SOURCE_FDP_PATH="${SOURCE_FDP_PATH:-}"
SINK_INITIAL_PAUSE_MS="${SINK_INITIAL_PAUSE_MS:-1000}"
SINK_HOLD_OPEN_MS="${SINK_HOLD_OPEN_MS:-1000}"
SINK_READ_DELAY_MS="${SINK_READ_DELAY_MS:-0}"
SINK_RECV_BUFFER_BYTES="${SINK_RECV_BUFFER_BYTES:-1024}"
SEND_CHUNK_BYTES="${SEND_CHUNK_BYTES:-2032}"
STATUS_POLL_INTERVAL_SEC="${STATUS_POLL_INTERVAL_SEC:-0}"
WAIT_FLUSH_MS="${WAIT_FLUSH_MS:-15000}"
SOURCE_REPEAT_COUNT="${SOURCE_REPEAT_COUNT:-1}"
COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS="${COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS:-10}"
...
SINK_HOLD_OPEN_MS="${SINK_HOLD_OPEN_MS}" \
...
COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS="${COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS}" \
python3 - <<'PY'
...
sink_hold_open_ms = int(os.environ["SINK_HOLD_OPEN_MS"])
...
comm_node_downlink_v3_drain_delay_ms = int(os.environ["COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS"])
...
def sample_downlink_status(csp_sub_port: int, csp_pub_port: int) -> dict[str, object]:
    result = subprocess.run(
        [
            str(bin_dir / "comm_downlink_v3_status_probe"),
            "--local-node",
            "7",
            "--target-node",
            "5",
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "comm_downlink_v3_status_probe failed "
...
class StatusCollector:
    def __init__(self, csp_sub_port: int, csp_pub_port: int, log_path: pathlib.Path) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="node5-downlink-v3-transport-status", daemon=True)
    ...
    def max_queue_slots(self) -> int:
        return max([int(sample["drainQueuedFrames"]) for sample in self.samples] or [0])
...
class SlowSink:
    def __init__(self,
        ...
        self._thread = threading.Thread(target=self._run, name="node5-downlink-v3-slow-sink", daemon=True)
        ...
        try:
            ...
            with self.output_path.open("wb") as handle:
                ...
                if self.hold_open_ms > 0:
                    time.sleep(self.hold_open_ms / 1000.0)
        except Exception as exc:  # noqa: BLE001
            self._error = exc
        finally:
            sock.close()
...
try:
    ...
    processes.append(
        start_managed_process(
            ...
            env={**runtime_env.copy(), "COMM_NODE_DOWNLINK_V3_DRAIN_DELAY_MS": str(comm_node_downlink_v3_drain_delay_ms)},
            cwd=root_dir,
    ...
    if initial_status is None:
        raise RuntimeError("Timed out waiting for node-5 DOWNLINK_CONTROL_V3 STATUS availability")
    ...
    if sender.returncode != 0:
        ...
        raise RuntimeError(
            "comm_downlink_v3_sender failed "
            f"returncode={sender.returncode} stderr={sender.stderr.strip()}"
        )
    sender_stdout_log.write_text(sender.stdout, encoding="utf-8")
    sender_stderr_log.write_text(sender.stderr, encoding="utf-8")
    sender_stdout_lines = [line for line in sender.stdout.splitlines() if line.strip()]
    if not sender_stdout_lines:
        raise RuntimeError("comm_downlink_v3_sender produced no stdout")
    sender_stats = json.loads(sender_stdout_lines[-1])
    ...
    collector_gap = collector.observed_gap() if collector is not None else False
    collector_max_gap = collector.max_gap() if collector is not None else 0
    collector_max_queue_slots = collector.max_queue_slots() if collector is not None else 0
    if not collector_gap and int(sender_stats["maxGapBytes"]) <= 0:
        raise RuntimeError(
            "Did not observe acceptedBytes > flushedBytes during transport-only hosted probe; "
            f"statusMaxGap={collector_max_gap} senderMaxGap={int(sender_stats['maxGapBytes'])} "
            f"maxQueueSlots={collector_max_queue_slots} "
            f"samples={len(collector.samples) if collector is not None else 0}"
        )

    print("node5-comm-csp-downlink-v3-transport-hosted-probe: PASS")
    print("formal-verdict=node5-comm-csp-downlink-v3 transport-hosted")
    ...
    print(
        "pipeline-gap=PASS "
        f"status-max-gap-bytes={collector_max_gap} "
        f"status-max-queued-slots={collector_max_queue_slots} "
